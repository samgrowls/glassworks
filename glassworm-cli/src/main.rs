//! GlassWorm - Unicode Attack Scanner CLI
//!
...
//! - Homoglyph attacks
//! - GlassWorm decoder patterns

use clap::{Parser, ValueEnum};
use colored::Colorize;
use glassworm_core::{
    scan, DecodedPayload, PayloadClass, Severity, UnicodeCategory, UnicodeFinding,
};
use serde::Serialize;
use std::fs;
...
/// GlassWorm - Detect invisible Unicode attacks in source code
#[derive(Parser, Debug)]
#[command(name = "glassworm")]
...
struct Args {
    ...
    #[arg(long, default_value = "js,mjs,cjs,ts,tsx,jsx,py,rs,go,java,rb,php,sh,bash,zsh,yml,yaml,toml,json,xml,md,txt")]
    extensions: String,

    /// Directories to exclude (comma-separated)
    #[arg(long, default_value = ".git,node_modules,target,__pycache__,.venv,vendor")]
    exclude: String,
}
...
fn main() {
    ...
    for file in &files {
        if let Ok(content) = fs::read_to_string(file) {
            let findings = scan(&content, &file.to_string_lossy());
    ...
    if !args.quiet {
        match args.format {
            OutputFormat::Pretty => output_pretty(&filtered_findings, &files, duration),
...
fn output_pretty(findings: &[UnicodeFinding], files: &[PathBuf], duration: std::time::Duration) {
    if findings.is_empty() {
        println!(
            "{}",
            "✓ No Unicode attacks detected".green().bold()
        );
        println!(
...
fn print_finding(finding: &UnicodeFinding, level: &str, color: &str) {
    ...
    println!("  Type: {}", finding.category.as_str().replace('_', " ").bold());
...
fn print_decoded_payload(payload: &DecodedPayload) {
    ...
    println!(
        "  {}",
        format!("Entropy: {:.2} bits/byte", payload.entropy)
    );
    println!(
        "  {}",
        format!("Classification: {}", payload.payload_class.description())
    );
...
fn output_json(findings: &[UnicodeFinding], files: &[PathBuf], duration: std::time::Duration) {
...
fn output_sarif(findings: &[UnicodeFinding], _files: &[PathBuf], _duration: std::time::Duration) {
    let sarif = SarifOutput {
        ...
        runs: vec![SarifRun {
            tool: SarifTool {
                driver: SarifDriver {
                    name: "glassworm".to_string(),
                    version: env!("CARGO_PKG_VERSION").to_string(),
                    information_uri: "https://github.com/PropertySightlines/glassworm".to_string(),
                    rules: vec![
                        SarifRule {
                            id: "GW001".to_string(),
                        ...
                        SarifRule {
                            ...
                            short_description: SarifMessage {
                                text: "GlassWorm-style decoder function pattern".to_string(),
                            },
            ...
            results: findings.iter().map(|f| SarifResult {
                rule_id: match f.category {
                    UnicodeCategory::SteganoPayload => "GW001",
                    UnicodeCategory::DecoderFunction => "GW002",
                    UnicodeCategory::InvisibleCharacter => "GW003",
                    UnicodeCategory::BidirectionalOverride => "GW004",
                    _ => "GW003",
                }.to_string(),
