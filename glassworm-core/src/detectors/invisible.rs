...
impl InvisibleCharDetector {
    ...
    fn is_emoji_context(&self, line: &str, char_pos: usize) -> bool {
        ...
        if char_pos > 0 {
            ...
            if (prev_cp >= 0x1F300 && prev_cp <= 0x1F9FF)
                || (prev_cp >= 0x2600 && prev_cp <= 0x26FF)
                || (prev_cp >= 0x2700 && prev_cp <= 0x27BF)
            {
                return true;
            }
        }
        ...
        if char_pos < chars.len() - 1 {
            ...
            if (next_cp >= 0x1F300 && next_cp <= 0x1F9FF)
                || (next_cp >= 0x2600 && next_cp <= 0x26FF)
                || (next_cp >= 0x2700 && next_cp <= 0x27BF)
            {
                return true;
            }
        }
