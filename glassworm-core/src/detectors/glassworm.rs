...
//!
//! Specialized detector for Glassworm attack patterns including:
...
use crate::finding::{Severity, SourceLocation, UnicodeCategory, UnicodeFinding};
...
impl GlasswormDetector {
    ...
    pub fn detect(&self, content: &str, file_path: &str) -> Vec<UnicodeFinding> {
        ...
        // 4. Detect Glassworm patterns (original functionality)
        findings.extend(self.detect_glassworm_patterns(content, file_path));
    ...
    fn detect_stego_runs(&self, content: &str, file_path: &str) -> Vec<UnicodeFinding> {
        let mut findings = Vec::new();
        ...
        for (start_offset, end_offset, codepoint_count) in runs {
            ...
            let finding = UnicodeFinding {
                ...
                raw_bytes: None,
                category: UnicodeCategory::SteganoPayload,
                severity,
                description,
                ...
                references: vec![
                    "https://www.aikido.dev/blog/glassworm-returns-unicode-attack-github-npm-vscode".to_string(),
                ],
    ...
    fn detect_pipe_delimiter(&self, content: &str, file_path: &str) -> Vec<UnicodeFinding> {
        let mut findings = Vec::new();
                ...
                if i + 1 < chars.len() && is_vs_codepoint(chars[i + 1].1) {
                    ...
                    if vs_count >= 4 {
                        ...
                        let finding = UnicodeFinding {
                            ...
                            character: String::new(),
                            raw_bytes: None,
                            category: UnicodeCategory::PipeDelimiterStego,
                            severity: Severity::Critical,
                            description: format!(
                                "Pipe delimiter steganography detected: {} VS codepoints after '|' (npm variant)",
                                vs_count
                            ),
                            remediation: "This is the npm GlassWorm variant. VS codepoints after \
    ...
    #[cfg(feature = "regex")]
    fn detect_decoder_functions(&self, content: &str, file_path: &str) -> Vec<UnicodeFinding> {
        let mut findings = Vec::new();
                ...
                if let Some(m) = pattern.find(line) {
                    ...
                    if has_codepointat {
                        let finding = UnicodeFinding {
                            ...
                            character: String::new(),
                            raw_bytes: None,
                            category: UnicodeCategory::DecoderFunction,
                            severity: Severity::High,
                            description: "GlassWorm-style decoder function detected: codePointAt \
    ...
    #[cfg(not(feature = "regex"))]
    fn detect_decoder_functions(&self, _content: &str, _file_path: &str) -> Vec<UnicodeFinding> {
        Vec::new()
    }

    /// Detect original GlassWorm patterns
    #[cfg(feature = "regex")]
    fn detect_glassworm_patterns(&self, content: &str, file_path: &str) -> Vec<UnicodeFinding> {
        let mut findings = Vec::new();
        ...
        if indicators.len() >= 2 {
            ...
            for (line_num, col, indicator_type) in indicators {
                ...
                let finding = UnicodeFinding {
                    ...
                    character: String::new(),
                    raw_bytes: None,
                    category: UnicodeCategory::GlasswormPattern,
                    severity,
                    description: format!(
    ...
    #[cfg(not(feature = "regex"))]
    fn detect_glassworm_patterns(&self, _content: &str, _file_path: &str) -> Vec<UnicodeFinding> {
        Vec::new()
    }
...
mod tests {
    ...
    fn test_stego_run_detection() {
        ...
        assert!(findings.iter().any(|f| f.category == UnicodeCategory::SteganoPayload));
    }
    ...
    fn test_pipe_delimiter_detection() {
        ...
        assert!(findings.iter().any(|f| f.category == UnicodeCategory::PipeDelimiterStego));
    }
    ...
    fn test_decoder_function_detection() {
        ...
        assert!(findings.iter().any(|f| f.category == UnicodeCategory::DecoderFunction));
    }
